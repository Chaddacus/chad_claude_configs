...
## Focused evidence and tool output

A later matched small-review screen found no benefit from putting all source into a separate evidence bundle: both reviewers found the defect and used two tool rounds; the bundle used 1.6% more input. Direct, bounded initial reads remain preferred when the relevant paths are already known. Guidance excerpts remain useful; they are distinct from duplicating all source files into another artifact.

Return useful command output and necessary status without serializing redundant transport details. Keep verbose logs complete on disk and return a concise, faithful diagnostic with its evidence path. Leave short useful outputs short. A fixture-specific log example reduced 2,680 output bytes to a 583-byte diagnostic while retaining failure names, source location, exception, counts, exit status, command, and full-log path. This is an output-size observation, not a measured model-token or cost improvement. No generic error parser, mandatory evidence-bundle format, or new execution framework was added.

## Context maintenance and stopping point

Keep `spec.md` as the current operational reference and move superseded execution notes to linked history. Update existing current-state sections rather than appending competing summaries. A rapid maintenance pass reduced this repository's spec from 4,882 to 3,144 bytes (35.6%) while retaining current capabilities, configuration, verification, and limitations. The separate local evaluation spec fell from 11,395 to 3,119 bytes (72.6%); its original text remains linked verbatim. These are artifact-size reductions, not measured session-token or cost savings.

The same pass retained the existing native budgets: a model-free desktop preview at 1,600 tokens was 21,164 bytes versus 22,760 at 2,000, but all 56 skill descriptions became shorter. Keeping names and paths alone does not prove routing quality. No live budget was changed. Removing every line-range label from a representative 8,274-byte excerpt would save at most 98 bytes (1.2%), so the reader remains unchanged. No legacy startup hooks were present in the inspected Claude settings.

The stopping point is evidence-based: remaining savings would require losing routing information, weakening evidence, or adding complexity for a small return. No lower reasoning setting, reduced testing requirement, hard output cap, removed capability, or new optimization framework was adopted. Revisit on observed repeated reads, routing failures, context growth, changed tools, or a new workload rather than rerunning broad benchmarks routinely. Runtime context and skill behavior are described in the [official configuration reference](https://learn.chatgpt.com/docs/config-file/config-reference) and [skill documentation](https://learn.chatgpt.com/docs/build-skills).
