...
For guidance excerpts, use `python3.11 ~/.codex/scripts/read-guidance.py --list` to discover headings, then request exact `guide:Section heading` selectors. This prints requested sections and their document introductions with source paths, line ranges, and hashes. Reuse its output rather than rereading complete guides; retrieve additional sections when their concern is reached.
