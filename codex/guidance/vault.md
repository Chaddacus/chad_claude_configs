...
Read its `llms.txt` for the navigation entry points. For adding or updating knowledge, use the `vault-knowledge` skill at `~/.codex/skills/vault-knowledge/SKILL.md`; its canonical entry standard and template live in the vault's `schema/` directory. If skill discovery has not refreshed, read that file directly.
